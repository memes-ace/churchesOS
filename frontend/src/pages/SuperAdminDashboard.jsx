...
import { Users, Church, DollarSign, TrendingUp, Eye, CheckCircle, XCircle, Clock, MoreVertical, Search, Filter, X, Shield, Bell, BarChart3, Globe, Star, AlertTriangle } from 'lucide-react'
...
const planConfig = {
  ...
  Starter: { bg: '#DBEAFE', text: '#1E40AF', price: 150 },
  Growth: { bg: '#EDE9FE', text: '#5B21B6', price: 450 },
  Enterprise: { bg: '#FEF9C3', text: '#854D0E', price: 850 },
}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {tab === 'overview' && (
            <div className="space-y-5">
              ...
              <div>
                <p className="text-sm font-bold text-gray-700 mb-3">Account Status</p>
                <div className="grid grid-cols-3 gap-2">
                  {['Active', 'Suspended', 'Pending'].map(s => (
                    <button key={s} onClick={() => setStatus(s)}
                      ...
                      style={{
                        borderColor: status === s ? statusConfig[s].text : '#E5E7EB',
                        background: status === s ? statusConfig[s].bg : 'white',
                        color: status === s ? statusConfig[s].text : '#6B7280'
                      }}>
          ...
          {tab === 'billing' && (
            <div className="space-y-4">
              ...
              <p className="text-sm font-bold text-gray-700 mb-2">Change Plan</p>
...
export default function SuperAdminDashboard() {
  ...
  const handleStatusChange = (id, status) => {
    setChurchList(prev => prev.map(c => c.id === id ? { ...c, status } : c))
  }
              ...
              <div className="flex items-center gap-6">
                <ResponsiveContainer width={160} height={160}>
                  <PieChart>
                    ...
                    <Tooltip formatter={v => [v + ' churches']} />
                  </PieChart>
                </ResponsiveContainer>
                  ...
                  {planData.map(p => (
                    <div key={p.name} className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-xs text-gray-400 ml-1">churches</span>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <div key={c.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: '#FEF9C3' }}>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => handleStatusChange(c.id, 'Suspended')}
                          className="px-3 py-1.5 rounded-lg text-xs font-medium text-white" style={{ background: '#DC2626' }}>
                          Reject
                        </button>
                      </div>
                    </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map(c => (
                  <tr key={c.id} className="table-row cursor-pointer" onClick={() => setSelected(c)}>
                    ...
                    <td className="py-4 px-4">
                      <button className="text-xs font-medium px-3 py-1.5 rounded-lg" style={{ background: '#EEF2FF', color: '#1B4FD8' }}>
                        Manage
                      </button>
                    </td>
                  </tr>
            ...
            <div className="space-y-3">
              {churchList.filter(c => c.revenue > 0).sort((a, b) => b.revenue - a.revenue).map(c => (
                <div key={c.id} className="flex items-center gap-4">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ background: '#1B4FD8' }}>
                    {c.name.split(' ').map(w => w[0]).slice(0,2).join('')}
                  </div>
                  <div className="flex-1 min-w-0">
      ...
      {activeTab === 'plans' && (
        <div className="space-y-6 fade-in">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { plan: 'Free', price: 0, features: ['Up to 100 members', 'Basic attendance', 'Member profiles', 'Prayer requests'], color: '#6B7280', bg: '#F3F4F6', churches: churchList.filter(c => c.plan === 'Free').length },
              { plan: 'Starter', price: 150, features: ['Up to 500 members', 'All Free features', 'Finance tracking', 'Events module', 'SMS (100/mo)'], color: '#1B4FD8', bg: '#EEF2FF', churches: churchList.filter(c => c.plan === 'Starter').length },
              { plan: 'Growth', price: 450, features: ['Up to 2000 members', 'All Starter features', 'Ministries & Cell Groups', 'Song Library', 'SMS (500/mo)', 'Reports'], color: '#7C3AED', bg: '#EDE9FE', churches: churchList.filter(c => c.plan === 'Growth').length },
              { plan: 'Enterprise', price: 850, features: ['Unlimited members', 'All Growth features', 'Custom branding', 'Priority support', 'Unlimited SMS', 'API access', 'Multi-branch'], color: '#F59E0B', bg: '#FEF9C3', churches: churchList.filter(c => c.plan === 'Enterprise').length },
            ].map(p => (
