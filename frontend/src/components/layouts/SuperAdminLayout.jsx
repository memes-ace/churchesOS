      ...
      <div className="px-5 py-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: '#1B4FD8' }}>
            <span className="text-white font-bold" style={{ fontFamily: 'Cormorant Garamond' }}>C</span>
          </div>
          <div>
            <p className="text-white font-bold" style={{ fontFamily: 'Cormorant Garamond' }}>ChurchesOS</p>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {navItems.map(({ label, path, icon: Icon, exact }) => (
          <NavLink key={path} to={path} end={exact} onClick={() => setOpen(false)}
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition ${isActive ? 'bg-white/15 text-white' : 'text-white/55 hover:bg-white/10 hover:text-white'}`
            }>
            <Icon size={16} />
            <span>{label}</span>
          </NavLink>
    ...
    <div className="flex h-screen overflow-hidden" style={{ background: '#F8FAFF' }}>
        ...
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative w-60 flex flex-col" style={{ background: '#0F172A' }}>
            <button onClick={() => setOpen(false)}
              className="absolute top-4 right-4 p-2 text-white/40 hover:bg-white/10 rounded-lg">
              <X size={18} />
            </button>
            <Nav />
          </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="lg:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-gray-100">
          ...
          <span className="font-bold text-gray-800" style={{ fontFamily: 'Cormorant Garamond' }}>ChurchesOS Admin</span>
          <div className="w-9" />
        </header>
